...
os.environ['CUDA_VISIBLE_DEVICES']= '0,1,2,3' #,,1
...
#co_neighbor=12000
max_epoch=240 
acc_file_name=  r'clf_score_mag_0LP1NC1T_240mul4_random.txt'
label_known_rate=0.01
train_rate=0.8
test_rate= 1
model_path=r'/home/xiezheng/Datasets/Sci_Papers/model_mag_0LP1NC1T240mul4_random'
...
labels = set(lines)
all_papers = [i for i in range(0, num_paper)] 
dic_labels = dict()
for i, x in enumerate(labels):
    dic_labels[x] = i 
node_origin_label = lines
node_label = [] 
for i in range(0, len(lines)):
    node_label.append(dic_labels[lines[i]]) 
num_class = len(labels)
print('labels', len(labels), 'num_node', len(lines),label_known_rate,max_epoch) 
i = 0
...
for line in lines:
    ...
    #u_bine[i] = [float(x) for x in line]
    i = i + 1
i = 0
...
row_0,col_0,data_0=A1.row, A1.col, A1.data
...
#    if data_0[i]>co_neighbor and row_0[i]>col_0[i]: 
#        row_1.append(row_0[i])
#        col_1.append(col_0[i])
#print ('cocit edge',len(row_1))      
...
#u_class=[]
#print ('hetero_graph.num_nodes',hetero_graph.num_nodes('class')  )
#for  i in range(0, hetero_graph.num_nodes('class')   ):
#    temp=u[node_classes[i][0]]
#    for j in range(1, len(node_classes[i])):
#            temp=u[node_classes[i][j]]+temp 
#    u_class.append(temp /len(node_classes[i]))
#u_class=torch.stack(u_class)
#hetero_graph.nodes['class'].data['features']=u_class  
...
if __name__ == '__main__':
    ...
    with open(acc_file_name, 'w') as fw:
        for i in range(0,5):        
            ...
            mp.spawn(run1.run, args=(list(range(num_gpus)),  model_lp,    hetero_graph ,   train_mask, max_epoch ),  nprocs=num_gpus, join=True )
            torch.save(model_lp.state_dict(),model_path) 
            device='cpu'
