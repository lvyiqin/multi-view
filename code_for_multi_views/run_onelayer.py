...
def run(proc_id,device_ids,  model_lp,  hetero_graph ,  train_mask, 
    ...
    model_lp = DDP(model_lp, device_ids=[device],  output_device=torch.device('cuda:' + str(device_ids[-1] )),find_unused_parameters=True ) #, device_ids=device_ids   ,find_unused_parameters=True
    ...
    model_lp.eval()
    if device==torch.device('cuda:' + str(device_ids[-1])):
        print ('aaaa') 
        #
