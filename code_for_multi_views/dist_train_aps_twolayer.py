...
max_epoch=2400
acc_file_name=  r'clf_score_aps_0LP1N10Tmul4_2400_random.txt'
label_known_rate=0.1
train_rate=0.8
test_rate= 1
model_path=r'/home/xiezheng/Datasets/Sci_Papers/model_aps_0LP1NC10Tmul4_2400__random'
...
for line in lines:
    ...
    #u_bine[shuffle_ix[i]] = [float(x) for x in line] #打乱后的新编号
    i = i + 1
i = 0
...
if __name__ == '__main__':
    ...
    with open(acc_file_name, 'w') as fw:
        ...
        for i in range(0,2):
            model_lp = Model_LP(n_features,512,  num_class, hetero_graph.etypes).to(torch.device('cuda'))      
            model_lp = DDP(model_lp, find_unused_parameters=True )     
            mp.spawn(run_twolayer.run, args=(list(range(num_gpus)),  model_lp,    hetero_graph ,   train_mask, max_epoch ),  nprocs=num_gpus, join=True )
