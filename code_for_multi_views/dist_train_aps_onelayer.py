...
class StochasticOneLayerRGCN(nn.Module):
...
def evaluate(model, graph, features, labels, mask):
    model.eval()
    with torch.no_grad(): 
        logits = model.module.rgcn([graph,graph,graph], features)['paper'] #module.
        logits = logits[mask]
        labels = labels[mask]
...
class Model_LP(nn.Module):
    def __init__(self, in_features,    out_features, 
                 etypes):
        super().__init__()
        self.rgcn = StochasticOneLayerRGCN(
            in_features,   out_features, etypes)
        self.pred = ScorePredictor()

    def forward(self, positive_graph, negative_graph, blocks,  x,  output_labels,train_mask, device,task_tag):
        if task_tag==1:
            x0 = self.rgcn(blocks, x) 
            dic_b_ref=dict()
            temp=0
            for i1 in blocks[-1].ndata[dgl.NID]['ref'].tolist():
                dic_b_ref[i1]=temp
                temp=temp+1
            dic_b_paper=dict() #key是原来的节点标号，value是block的节点标号
            temp=0
            for i1 in blocks[-1].ndata[dgl.NID]['paper'].tolist():
                dic_b_paper[i1]=temp
                temp=temp+1 
            dic_b_class=dict() #key是原来的节点标号，value是block的节点标号
            temp=0
            for i1 in blocks[-1].ndata[dgl.NID]['class'].tolist():
                dic_b_class[i1]=temp
                temp=temp+1     
        ...
        if   task_tag==2:            
            ...
            output_predictions = self.rgcn(blocks, x) ['paper'] 
...
max_epoch=2400
acc_file_name=  r'clf_score_aps_0LP1N10Tmul4_2400_v1.txt'
label_known_rate=0.1
train_rate=0.8
test_rate= 1
model_path=r'/home/xiezheng/Datasets/Sci_Papers/model_aps_0LP1NC10Tmul4_2400_v1'
shuffle_ix_path=r'/home/xiezheng/Datasets/Sci_Papers/shuffle_ix_path' 
#with open(r'/home/xiezheng/Datasets/Sci_Papers/MAG/aps_papers_20_label_first.txt')as fl:
...
if os.path.exists(shuffle_ix_path):
    shuffle_ix=[]
    with open(shuffle_ix_path, 'r') as fr:
        huffle_ixs = fr.readlines() 
    for i in huffle_ixs:
        shuffle_ix.append(int(i))
else:
    random.seed(3)
    shuffle_ix = np.random.permutation(np.arange(num_paper))
    with open(shuffle_ix_path, 'w') as fw:
        for i in range(0,len(shuffle_ix)):
            fw.write('%d\n' % shuffle_ix[i]) 
 
node_retag=dict()
for i, x in enumerate(shuffle_ix):
    node_retag[x]=i    
 
labels = set(lines)
all_papers = shuffle_ix
dic_labels = dict()
for i, x in enumerate(labels): #给标签编号
    dic_labels[x] = i 
    
 
node_origin_label =['0' for i in range(0,num_paper)]
for i in range(0,num_paper):
        node_origin_label[shuffle_ix[i]]=lines[i]   
...
with open(r'/home/xiezheng/Datasets/Sci_Papers/APS/APS_title_bert.txt')as fo:     
    lines = fo.readlines()
...
for line in lines:
    ...
    u_bine[shuffle_ix[i]] = [float(x) for x in line]
    i = i + 1
i = 0
...
with open(r'/home/xiezheng/Datasets/Sci_Papers/APS/cite_uvw.txt')as fu:
    while 1:
        ...
        if line == '':
            break
        line = line.strip().split('\t')
        if int(line[0][1:])<num_paper:
            s.append( shuffle_ix[  int(line[0][1:])])
            e.append(  int(line[1][1:]) )
...
if __name__ == '__main__':
    ...
    with open(acc_file_name, 'w') as fw:
        ...
        for i in range(0,7):
            model_lp = Model_LP(n_features,  num_class, hetero_graph.etypes).to(torch.device('cuda:' + str(device_ids[-1] )))      
            model_lp = DDP(model_lp, find_unused_parameters=True )     
            mp.spawn(run_onelayer.run, args=(list(range(num_gpus)),  model_lp,    hetero_graph ,   train_mask, max_epoch ),  nprocs=num_gpus, join=True )
            ...
            node_embeddings = model_lp.module.rgcn([hetero_graph,hetero_graph,hetero_graph], node_features )['paper']
                #module.
